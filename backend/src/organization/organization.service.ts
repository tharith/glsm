...
export class OrganizationService {
  ...
  async findAll(q: OrgUnitQueryDto) {
    ...
    return this.prisma.orgUnit.findMany({
      ...
      include: {
        parent: {
          select: { id: true, code: true, nameEn: true, nameKh: true },
        },
    ...
    if (dto.parentId) {
      ...
      if (hierarchy.indexOf(dto.type) <= hierarchy.indexOf(parent.type))
        throw new BadRequestException(
          `A ${dto.type} cannot be child of ${parent.type}`,
        );
    }
  ...
  async remove(id: string) {
    ...
    const [children, users] = await Promise.all([
      ...
      this.prisma.user.count({
        where: { orgUnitId: id, isActive: true, isDeleted: false },
      }),
    ]);
    if (children > 0)
      throw new BadRequestException(
        `Has ${children} active child unit(s). Deactivate children first.`,
      );
    if (users > 0)
      throw new BadRequestException(
        `Has ${users} active user(s). Reassign users first.`,
      );
    return this.prisma.orgUnit.update({
      where: { id },
      data: { isActive: false },
  ...
  async getStaff(id: string, includeChildren = false) {
    ...
    const ids = includeChildren
      ? [id, ...(await this.getDescendants(id))]
      : [id];
    return this.prisma.user.findMany({
      ...
      select: {
        ...
        email: true,
        position: {
          select: { code: true, nameEn: true, nameKh: true, rank: true },
        },
        orgUnit: { select: { code: true, nameEn: true } },
  ...
  private async checkCircular(id: string, newParentId: string) {
    ...
    if (descendants.includes(newParentId))
      throw new BadRequestException(
        "Circular reference: new parent is a descendant",
      );
  }
}

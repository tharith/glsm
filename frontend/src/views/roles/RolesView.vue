  ...
  <div>
      ...
      <v-col cols="12" md="8">
        <v-card rounded="xl" elevation="1" v-if="selectedRole">
          ...
          <v-card-text class="pa-5">
            ...
            >
              <div
                class="text-caption font-weight-bold text-primary mb-2 text-uppercase"
              >
                📦 {{ module }}
              </div>
              <v-row dense>
    ...
    <v-card rounded="xl" elevation="1" class="mt-4 pa-5">
      ...
      <div class="text-caption text-medium-emphasis mb-4">
        PDF spec: Grant specific permission directly to a user without changing
        their role. Example: EMPLOYEE + report:view → can view reports without
        being HR_OFFICER.
      </div>
      <v-row dense align="center">
        ...
        <v-col cols="12" sm="2">
          <v-select
            v-model="overrideGrant"
            label="Action"
            :items="[
              { title: 'Grant ✅', value: true },
...
const ROLE_COLORS: Record<string, string> = {
  SYSTEM_ADMIN: "purple",
  INSTITUTION_HEAD: "deep-purple",
  DIRECTOR_GENERAL: "indigo",
  DEPARTMENT_CHIEF: "primary",
  OFFICE_CHIEF: "teal",
  HR_OFFICER: "amber",
  EMPLOYEE: "grey",
};
...
const selectedRolePermIds = computed(
  () =>
    new Set(
      (selectedRole.value?.permissions || []).map(
